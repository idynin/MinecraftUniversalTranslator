...
public class GoogleTranslatorAdapterTest {
  ...
  private String outPhrase = "Тест предложение 1 ? Тест приговор два.";
  ...
  public final void testGetDetectedLanguage() {
    ...
    assertEquals(Thread.currentThread().getStackTrace()[1].getMethodName(), realSourceLang,
        result.getFromLanguage());
  }
  ...
  public final void testGetSuportedLanguages() {
    assertTrue("testGetSuportedLanguages",
        gta.getSupportedLanguages().containsAll(EnumSet.allOf(Language.class)));
  }
  ...
  public final void testTranslate() {
    ...
    assertEquals("ExpectedTranslation", outPhrase, output.getTranslatedText());
  }
  ...
  public final void testTranslateList() {
    List<TranslationQuery> sourceList = new ArrayList<TranslationQuery>() {
      ...
      {
        add(new TranslationQuery("test message", Language.AUTOMATIC_DETECTION, Language.RUSSIAN));
        add(new TranslationQuery("test message 1! and also", Language.AUTOMATIC_DETECTION,
            Language.GERMAN));
        add(new TranslationQuery("mmmpf lol I'm silly.", Language.AUTOMATIC_DETECTION,
            Language.NORWEGIAN));
      }
    };
    List<Translation> expectedResults = new ArrayList<Translation>() {
      ...
      {
        add(new Translation("test message", "тестовое сообщение", Language.ENGLISH,
            Language.RUSSIAN));
        add(new Translation("test message 1! and also", "Test-Nachricht ein ! und auch",
            Language.ENGLISH, Language.GERMAN));
        add(new Translation("mmmpf lol I'm silly.", "mmmpf lol jeg er dum .", Language.ENGLISH,
            Language.NORWEGIAN));
      }
    };
