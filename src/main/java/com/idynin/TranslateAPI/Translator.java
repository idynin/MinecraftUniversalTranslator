...
public class Translator {
	...
	private InputStream performGet(URL url) {
		try {
	...
	public String translate(String sourcetext, Language sourceLang,
		...
		String translatedText = "translation error";
		...
		if (sourcetext != null) {
			...
			if (cache.contains(sourcetext)) {
				...
				lastDetectedLanguage = Language.getBestMatchByCode(result[0]);
				return result[1];
			}
			...
			try {
				...
				String urlString = "http://translate.google.com/translate_a/t?client=t&text="
						...
						+ "&tl="
						+ targetLang.code
						+ "&ie=UTF-8&oe=UTF-8&multires=1&otf=2&pc=3&ssel=3&tsel=3&uptl="
						+ targetLang.code + "&alttl=de&sc=1";
				...
				try {
					JsonElement je = new JsonParser()
							.parse(new InputStreamReader(performGet(longURL),
									CharEncoding.UTF_8));

					System.out.println(je);
					lastDetectedLanguage = Language.getBestMatchByCode(je
							.getAsJsonArray().get(2).getAsString());
					System.out.println("Detected Language: "
							+ lastDetectedLanguage);

					translatedText = je.getAsJsonArray().get(0)
							.getAsJsonArray().get(0).getAsJsonArray().get(0)
							.getAsString();

					cache.store(sourcetext, lastDetectedLanguage.code,
							translatedText);
				} catch (JsonIOException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
