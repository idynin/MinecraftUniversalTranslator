...
public class GoogleTranslatorAdapter extends TranslatorAdapter {
  ...
  private static final EnumSet<Language> supportedLanguages = EnumSet.allOf(Language.class);

  private static URL constructURL(TranslationQuery tq) throws MalformedURLException {
    return constructURL(tq.getSourceText(), tq.getFromLanguage(), tq.getToLanguage());
  }

  private static URL constructURL(String text, Language sourceLanguage, Language targetLanguage)
      throws MalformedURLException {
    try {
      text = URLEncoder.encode(text, CharEncoding.UTF_8).replaceAll("\\+", "%20");
    } catch (UnsupportedEncodingException e) {
      // Better be supported...
      e.printStackTrace();
    }
    String constructedURLString = urlString;
    constructedURLString = StringUtils.replace(constructedURLString, "$${text}", text);
    constructedURLString =
        StringUtils.replace(constructedURLString, "$${sourceLanguage}", sourceLanguage.code);
    constructedURLString =
        StringUtils.replace(constructedURLString, "$${targetLanguage}", targetLanguage.code);
  ...
  public List<Translation> translate(List<TranslationQuery> queryList) {
    ...
    List<Translation> translations = new ArrayList<Translation>(queryList.size() + 1);
  ...
  @Override
  public Translation translate(String text, Language fromLanguage, Language toLanguage) {
    ...
    try {
      ...
      Translation result = parseTranslationFromJsonString(netHandler.get(translateURL));
  ...
  private Translation parseTranslationFromJsonString(String rawJson) {
    ...
    Iterator<JsonElement> partIterator = je.getAsJsonArray().get(0).getAsJsonArray().iterator();

    String translation = "";
    while (partIterator.hasNext()) {
      translation += partIterator.next().getAsJsonArray().get(0).getAsString();
    }

    return new Translation(null, translation, Language.getBestMatchByCode(je.getAsJsonArray()
        .get(2).getAsString()), null);
  }

  @Override
  public Translation translate(TranslationQuery query) {
    return translate(query.getSourceText(), query.getFromLanguage(), query.getToLanguage());
  }
